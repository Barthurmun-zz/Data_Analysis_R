...
#czytanie xls'a
piwa <- readxl::read_excel("PiwaWyniki.xlsx", 2)
...
met <- read.csv('Samochod.csv', header = T)

liczba_dni <- as.Date(met$Data.tankowania)
plot(liczba_dni)

#Skadanie kilku zapytan w jedno -> Potok
met$Data.tankowania %>% as.Date() %>% as.numeric()

tankowanie <- as.Date(met$Data.tankowania) 
licznik <- met$Licznik
regresja <- lm(formula = licznik ~ tankowanie)
plot(tankowanie, licznik)
c <- coefficients(regresja)
abline(c[2], c[1])

ggplot(data=met, aes(x=Cena.tankowania)) + geom_histogram()

cena <- met$Cena.tankowania
do_pelna <- met$Do.peÅ.na
klas_model <- glm(do_pelna ~ cena, family = 'binomial')
wyniki <- predict(klas_model, data.frame(cena=cena), type = 'response')
wyniki
